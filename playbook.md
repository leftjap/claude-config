...
→ backlog.md: https://raw.githubusercontent.com/leftjap/playbook/main/backlog.md

### 읽기 트리거 (크롤링)

| 트리거 | 조건 |
|---|---|
| 세션 시작 | 사용자가 playbook.md 업로드 직후, backlog.md를 크롤링하여 현황 파악 |
| 우선순위 판단 | 사용자가 "오늘 뭐 하지?" 등 작업 선택을 요청할 때 |
| 상태 점검 | GitHub 커밋과 백로그를 대조할 때 |
| 선행조건 확인 | 작업지시서 생성 시 "B-xx 후" 등 의존 관계를 확인할 때 |
| 백로그 ID 언급 | 사용자가 특정 B-xx를 언급했는데 직전 크롤링에 없으면 backlog-archive.md 크롤링 |

### 쓰기 트리거 (Claude Code 편집)

| 트리거 | 조건 |
|---|---|
| 작업지시서 종료 Step | 모든 작업지시서 마지막에 backlog.md 갱신 Step 포함 (상태 변경·메모 갱신) |
| 완료 이관 | 작업 완료 시 해당 행을 backlog-archive.md로 이동 후 backlog.md에서 삭제 |
| 교훈 연계 | L-13 해당 시 백로그 메모에 "N회 시도 미해결" 기재 |
| 즉석 변경 | 작업지시서 없이 발생하는 백로그 추가·상태 변경도 Claude Code가 직접 편집 |

### 세션 시작 최적화

사용자가 작업을 바로 지정하며 해당 B-xx ID를 명시한 경우, 전체 백로그 크롤링 대신 해당 항목의 존재만 확인하고 진행해도 된다. 전체 현황 파악이 필요한 경우에만 전문 크롤링한다.
